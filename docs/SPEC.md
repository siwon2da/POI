...
필수 매개변수는 기본값이 있는 매개변수보다 먼저 선언해야 한다. 순서가 잘못되면 P023과 수정 힌트를 표시한다.
한 함수 안의 매개변수 이름은 서로 달라야 하며, 중복 선언은 P024로 알려 준다.
...
`--types` 검사에서는 조건이 `Bool`, 실패 설명이 `Text`인지 확인한다(P406/P407).
`poi run` 에서는 `test` 블록을 정의만 하고 실행하지 않는다.

`--types`는 `if`, `while`, 삼항식의 조건도 `Bool`인지 확인한다(P408).
사용자 함수 호출에서는 필수 인자 누락(P409), 잘못된 이름 인자(P410), 중복 인자(P411)와 이름 인자의 타입(P403)도 확인한다.
타입이 선언된 매개변수의 기본값도 검사하며, 맞지 않으면 P412로 알려 준다. nullable 타입(`Int?` 등)은 `null` 기본값을 허용한다.
