...
> 버전 기준: **POI v1.7**. 공식: https://hagora.kr/poi/ · 저장소: https://github.com/siwon2da/POI
...
7. **변수 선언 키워드가 없다**: 그냥 `x = 10`. (`let`/`var` 없음. `const` 는 관례상 상수, 아직 재대입은 막지 않음.)
...
- **예약어**: `if else for in fn return const show ask use python try catch true false null
  is between and or not repeat as end raise assert match when`
...
for k in user.keys() { show k }
```

`while` 도 있다: `while n > 0 { n = n - 1 }`.
...
try {
    ...
    show "실패: {error}"          # error 는 사람이 읽기 좋은 문자열
}
