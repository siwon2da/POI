...
## v1.18 — 신뢰성·멀티파일·하온 ✅ (현재)

- [x] `poi.apps`와 `.poi` 리소스를 포함하는 wheel · 새 환경 설치 smoke
- [x] 하온 프로젝트 에이전트: 하위 `.poi` 탐색 · 상대 모듈 검사 · `src/main.poi` 실행
- [x] 전용 EXE 컨테이너 검사/복구와 IDLE 연결
- [x] 안전 모드 내부 속성 차단 · 플레이그라운드 비밀 환경 차단 · 레지스트리 압축 검증
- [x] `List<T>`/`Map<K,V>` 원소 추론 · 모든 경로 `return` 검사
입문
 │
...
- [x] **웹 폴더 열기** — 디렉토리 → server 파일 자동 실행 / 없으면 정적 서버
- [x] **하온 (로컬 에이전트)** — 규칙 기반 오토픽스(실제 컴파일러 구동) + Ollama LLM 모드(GPU 자동)
- [x] 설치 시 **바탕화면 바로가기 + 자동 실행**, `webserver.start()` non-blocking API
