...
## v1.18.6 — 2026-09-11  (LSP 연결 복구)

- 잘못된 JSON-RPC 한 건이나 비정상 `Content-Length`가 들어와도 LSP 프로세스 전체를 종료하지 않는다.
- 손상된 메시지만 건너뛰고 다음 정상 요청을 계속 처리하는 회귀 테스트를 추가했다.

## v1.18.5 — 2026-09-11  (LSP 진단 안정화)

- 린트·타입 진단 범위를 임의의 200칸이 아니라 실제 코드 줄 안으로 제한한다.
- 문서를 닫으면 남아 있던 진단을 즉시 지우고, 저장 시에도 최신 내용을 다시 검사한다.
- LSP 초기화 응답에 열기·닫기·전체 동기화·저장 지원을 명시했다.

## v1.18.4 — 2026-09-11  (IDLE 안전 중단)

- 실행 중단 시 CPython이 정확히 한 작업 스레드만 선택했는지 확인한다.
- 중단 API가 비정상적으로 여러 스레드를 건드리면 즉시 되돌려 IDLE 전체가 불안정해지거나 꺼지는 위험을 막는다.

## v1.18.3 — 2026-09-11  (미니하온 무변경 보호)

- 미니하온이 자동으로 고칠 수 없는 오류에서는 편집기 내용을 덮어쓰거나 파일을 수정됨 상태로 만들지 않는다.
- 실제 변경이 없을 때는 원본을 유지했다는 결과와 남은 오류를 정확히 안내한다.

## v1.18.2 — 2026-09-10  (IDLE 복구·미니하온)

- Tk 콜백 오류를 가로채 IDLE을 계속 실행하고 `~/.poi/idle-crash.log`에 복구 정보를 남긴다.
- 실행 오류가 발생하면 미니하온 자동 수정을 제안하고, 승인 시 컴파일러 기반 수정안을 편집기에 적용한다.
- 창 닫기 전에 미저장 파일과 실행 중인 프로그램을 확인하며 IDLE 자체 점검을 최신 도구명에 맞췄다.

## v1.18.1 — 2026-09-10  (IDLE 실행 중단 수정)

- 일반 POI 프로그램을 8초 뒤 강제로 끊던 IDLE 타이머를 제거했다.
- 사용자가 중단 버튼을 눌렀을 때만 실행을 멈추고 정확한 안내를 표시한다.

## v1.18.0 — 2026-09-10  (멀티파일·하온·신뢰성 릴리스)

- `poi.apps`와 IDLE 리소스를 wheel에 포함해 새 가상환경 설치에서도 IDLE·하온·사진 앱이 동작한다.
- 하온 에이전트가 프로젝트 하위의 여러 `.poi` 파일을 탐색하고, 상대 모듈 경로를 검사하며, `src/main.poi`를 진입점으로 실행한다.
- 전용 EXE의 `poi decompile`/IDLE 검사가 POI 컨테이너를 먼저 인식하고 스텁 전체를 무작정 풀지 않는다.
- 안전 모드 내부 속성 차단, 플레이그라운드 비밀 환경변수 제거, 교차 출처 실행 엔드포인트 차단을 추가했다.
- 레지스트리 설치에 HTTPS·SHA-256·압축 크기·파일 수·경로·심볼릭 링크 검증과 원자적 교체를 적용했다.
- 제네릭 타입(`List<Int>`, `Map<Text,Int>`)과 모든 경로 반환 검사를 보강했다.

## v1.17.0 — 2026-09-10  (POI 전용 EXE 패키저)

- 설치된 `poi.exe`와 POI IDLE에서 Python·pip·PyInstaller 없이 `.poi`를 단일 EXE로 만든다.
- 컴파일 최적화 바이트코드를 압축해 실행 스텁에 직접 넣고, 크기 제한·SHA-256 이중 검증·원자적 파일 교체를 적용했다.
- `poi verify 파일.exe`로 컨테이너 형식, 압축 데이터, 코드 무결성과 빌드 정보를 검사한다.
- `--obfuscate`, `--lock`, `--ask-password`, 콘솔/GUI 스텁을 전용 패키저에서도 지원한다.
- 손상된 길이·JSON·압축·바이트코드와 위험한 Windows 파일명을 거부하고 패키저 회귀 테스트를 추가했다.

## v1.16.8 — 2026-09-10  (중복 매개변수 진단)

- 함수 선언에서 같은 매개변수 이름을 두 번 쓰면 파서가 즉시 P024를 표시한다.
- 내부 Python 변환 오류(P098) 대신 중복된 이름과 서로 다른 이름을 쓰라는 해결 방법을 알려 준다.
- 오류 코드·위치·힌트를 검증하는 파서 회귀 테스트를 추가했다.

## v1.16.7 — 2026-09-10  (함수 매개변수 순서 진단)

- 기본값이 있는 매개변수 뒤에 필수 매개변수를 선언하면 파서가 즉시 P023을 표시한다.
- 내부 Python 변환 오류(P098) 대신 문제가 된 인자 이름과 올바른 수정 방법을 알려 준다.
- 정상 선언과 잘못된 선언을 검증하는 파서 단위 테스트를 추가했다.

## v1.16.6 — 2026-09-10  (기본값 타입 검사 · nullable 복구)

- 함수 매개변수의 기본값이 선언 타입과 다르면 P412로 실행 전에 알려 준다.
- 문서화된 nullable 타입 표기(`Int?`, `Text?` 등)의 `?` 토큰이 누락됐던 렉서 결함을 수정했다.
- nullable 타입은 `null` 기본값을 올바르게 허용하도록 회귀 테스트를 추가했다.

## v1.16.5 — 2026-09-10  (함수 호출 정적 검사)

- 사용자 함수 호출에서 기본값을 고려해 빠진 필수 인자를 P409로 알려 준다.
- 존재하지 않는 이름 인자(P410)와 위치·이름으로 중복 전달한 인자(P411)를 검사한다.
- 이름 인자도 선언 타입과 맞는지 기존 P403으로 확인한다.

## v1.16.4 — 2026-09-10  (제어 흐름 조건 정적 검사)

- `if`, `else if`, `while`, 삼항식의 조건이 `Bool`인지 `poi check --types`에서 검사한다(P408).
- 기존에 빠져 있던 `while` 본문도 정적 타입 검사 대상에 포함했다.
- 올바른 조건과 세 오류 경로를 검증하는 단위 테스트 4개를 추가했다.

## v1.16.3 — 2026-09-10  (`assert` 정적 타입 검사)

- `poi check --types`가 `assert` 조건의 `Bool` 타입을 검사한다(P406).
- 사용자 실패 설명은 `Text`인지 검사한다(P407).
- 올바른 조건과 두 오류 경로를 검증하는 단위 테스트 3개를 추가했다.

## v1.16.2 — 2026-09-10  (`assert` 사용자 실패 설명)

- `assert 조건, "설명"` 문법을 추가했다. 실패 오류에 설명과 원래 조건을 함께 표시한다.
- 설명 식은 조건이 실패할 때만 평가해 성공 경로의 부작용을 막는다.
- 기존 `assert 조건` 문법은 그대로 호환된다.

## v1.16.1 — 2026-09-10  (IDLE 버전 통일 · 하온 ChatGPT 바로 로그인)

- 코어, POI IDLE, 설치 마법사가 모두 루트 `VERSION` 기준의 같은 버전을 표시한다.
- 하온 패널을 처음 열 때 ChatGPT 로그인이 안 되어 있으면 OAuth 로그인 창을 즉시 연다.
- 하온 패널에 로그인 상태와 `GPT 로그인` 재시도 버튼을 추가했다.
